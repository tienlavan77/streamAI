// ui-panel.js — chạy trong isolated world (mặc định của content script)
// Trách nhiệm: tạo panel nổi hiển thị nội dung đang stream, kéo-thả được, có nút xóa/copy/thu gọn.
...
const AICapturerUI = (() => {
  ...
  function injectPanel() {
    ...
    panelEl.innerHTML = `
      <div id="ai-capturer-header">
        <span>🧩 AI Capturer <small id="ai-capturer-status"></small></span>
        <div>
      ...
      <pre id="ai-capturer-content">(chưa có dữ liệu)</pre>
    `;

    document.documentElement.appendChild(panelEl);
    contentEl = panelEl.querySelector('#ai-capturer-content');
    statusEl = panelEl.querySelector('#ai-capturer-status');
  ...
  function injectStyles() {
    if (document.getElementById('ai-capturer-style')) return;
    const style = document.createElement('style');
    style.id = 'ai-capturer-style';
    style.textContent = `
      #ai-capturer-panel {
        position: fixed;
        top: 80px;
      ...
      #ai-capturer-header small {
        font-weight: 400;
        color: #8a8a8a;
        margin-left: 6px;
      }
      #ai-capturer-header button {
        background: none;
        border: none;
        color: #ccc;
        cursor: pointer;
  ...
  function endStream(requestId, meta) {
    setStatus(meta?.codeBlockCount ? `xong · ${meta.codeBlockCount} code block` : 'xong');
    render();
  }
  ...
  return { injectPanel, startStream, appendChunk, endStream, setStatus };
})();

// Gán tường minh + báo sẵn sàng NGAY LẬP TỨC — không phụ thuộc DOMContentLoaded.
// bridge.js load cùng lúc và cần biết AICapturerUI đã tồn tại chưa; tách việc gán biến
// (đồng bộ, không cần DOM) ra khỏi việc chèn panel vào DOM (cần DOM sẵn sàng).
